...
template <class Mesh> ScalarField smooth_discrete_hyper_surface(Mesh &m) {
  ...
  for (uint pid = 0; pid < m.num_polys(); ++pid) {
    if (m.poly_data(pid).label == 1)
      field.set(pid, -field.vec_at(pid));
  }
  field.normalize();
